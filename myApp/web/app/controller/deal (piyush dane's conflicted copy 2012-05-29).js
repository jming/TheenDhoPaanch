...
Ext.define('myApp.controller.deal', {
    ...
    cardClicked: function (x) {
        ...
        var splicedCard = hands[currPlayer - 1][order];
        table[currHand - 1] = splicedCard;
        // console.log(table[currHand-1]);
        // console.log(c);
        Ext.getCmp('table'+ currHand).setHtml(table[currHand - 1][0].image);
        ...
        else
        {
            ...
            Ext.getCmp('transitionStage').setHtml("This concludes round " + roundNumber + ". Please pass the device to the winner, " + players[winner - 1] + ".");
            currHand = 1;
            currPlayer = winner;
        }
...
{
	if(cardthrown(0).suit == cardthrown(1).suit)
	{
		if(cardthrown(1).suit == cardthrown(2).suit)
		{
			if(cardthrown(0).number > cardthrown(1).number)
			{
				if(cardthrown(0).number > cardthrown(2).number) winner = 0;
				else winner = 2;
			}
			else
			{
				if(cardthrown(1).number > cardthrown(2).number) winner = 1;
				else winner = 2;
			}
		}
		else if(cardthrown(2).suit == trump && cardthrown(1).suit != trump) winner = 2;
		else 
		{
			if(cardthrown(0).number > cardthrown(1).number)
			{
	...
	if(cardthrown(0).suit == trump && cardthrown(0).suit != cardthrown(1).suit)
	{
		if(cardthrown(0).suit == cardthrown(2).suit)
		{
			if(cardthrown(0).number > cardthrown(2).number) winner = 0;
			else winner = 2;
		}
	...
	else
	{
		...
		else if(cardthrown(1).suit == trump && cardthrown(2) != trump) winner = 1;
		else if(cardthrown(1).suit == trump && cardthrown(2) == trump)
		{
			if(cardthrown(1).number > cardthrown(2).number) winner = 1;
			else winner = 2;
